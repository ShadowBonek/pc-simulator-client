...
const SearchSt = styled.form`
  ...
  .thumbnails-arrow-container {
    ...
    height: 30vw;
    display: grid;
    grid-template-columns: 10vw calc(100% - 20vw) 10vw;
...
const Search = () => {
  ...
  // !Handle Changes
  const handleChangeState = (e: React.ChangeEvent<HTMLInputElement> | React.ChangeEvent<HTMLSelectElement>) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await axios
      .put(`http://localhost:4000/${params.component}/${id}`, formData, {
        headers: {
  ...
  const fetchData = async ({ queryKey }: any) => {
    const { data } = await axios.get(`http://localhost:4000/${queryKey[0]}/${queryKey[1]}`);
    return data;
  };
  // !UseQuery
  ...
  return (
    <SearchSt onSubmit={handleSubmit}>
      <h1 className="title-add">Editar {params.component}</h1>
        ...
        {state &&
          components[`${params.component}`].map((i: any) => (
            <div className="input-label-container" key={i.key}>
              ...
              {i.inputType === "select" ? (
                <select name={i.key} className="input" value={state[i.key]} onChange={(e) => handleChangeState(e)}>
                  {i.options.map((o: any) => (
